...
describe('TournamentManager - WebSocket Communication', () => {
    ...
    it('should broadcast tournament updates to all participants when players join and the tournament starts', async function() {
        ...
        let tournamentState: Tournament = await waitForMessage(wsCreator, 'tournamentUpdate');
        expect(tournamentState.players).to.have.lengthOf(1);
        expect(tournamentState.players[0].nickname).to.equal('Alice');

        let updatePromise = waitForMessage(wsCreator, 'tournamentUpdate'); 
        await app.inject({
            method: 'POST',
        ...
        tournamentState = await updatePromise; 
        expect(tournamentState.players).to.have.lengthOf(2);
        expect(tournamentState.players.map(p => p.nickname)).to.include.members(['Alice', 'Bob']);

        updatePromise = waitForMessage(wsCreator, 'tournamentUpdate');
        await app.inject({
            method: 'POST',
            url: '/api/tournament/join',
            payload: { tournamentId },
        ...
        tournamentState = await updatePromise;
        expect(tournamentState.players).to.have.lengthOf(3);
        expect(tournamentState.players.map(p => p.nickname)).to.include.members(['Alice', 'Bob', 'Charlie']);
        ...
        await Promise.all([
            new Promise(resolve => wsPlayer2.on('open', resolve)),
            new Promise(resolve => wsPlayer3.on('open', resolve))
        ]);
