...
94. 패들과 공의 충돌이 불안정함 (프론트/백 로직 간 충돌일 가능성 있음) --> 공 무브먼트 해결... 이제는 패들과 공이 떨어져서 충돌하는 이유 알아보기 --> 값 중심으로

95. 생각 중
	- 참가자 혹은 생성자가 홈페이지로 이동을 해서도 해당 토너먼트 화면으로 접근할 수 있게 접근성 --> 이미 참가한 경우에만 open 버튼을 통해서 참가 가능

96. 토너먼트 로비에서 참가자의 leave 버튼이 작동 하지 않음 (api?)

97. 현재 ai는 단순히 공의 움직임만 추적하고, 특정한 비율에 따라서 공 따라가는 걸 포기함. --> 핸드 메이드 ai로직 만들기

98. 수정이후, 공이 직선으로만 튀어서 일직선 운동만 함....
