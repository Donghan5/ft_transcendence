...
89. 게임 관련 ui 해결 급함 --> 특히 스코어 보드 + 게임 승자 패자 깔끔하게 표시 --> 현재 해당 로컬 플레이어가 아닐 시 플레이어3 이런 식으로 표기함
	- 스코어 보드 업데이트 안됨, 플레이어 이름 표시 안됨, 결과 표시 창 로딩이 느림

90. walk-over 관련 대진표 생성 문제 --> 생성에는 문제가 없는데 walkover 로 표시가 됨

91. 토너먼트 종료 후, 토너먼트 결과 표시 --> 토너먼트 승자 화면에만 표시됨

92. 매 변경사항 마다 자연스럽게 반영 (페이지 새로고침 없이)

93. 게임 입장도 새로고침을 해야 입장됨
