#!/bin/bash
# infra/elk/security/setup-passwords.sh
set -e
...
curl -s -X PUT -u ${ELASTIC_USER}:${ELASTIC_PASSWORD} -H "Content-Type: application/json" \
  ...
  -d '{"cluster":["manage_index_templates","monitor","manage_ilm"],"indices":[{"names":["pong-logs-*","logstash-*"],"privileges":["write","create","create_index","manage","manage_ilm"]}]}' || true

echo "Creating logstash_internal user..."
curl -s -X POST -u ${ELASTIC_USER}:${ELASTIC_PASSWORD} -H "Content-Type: application/json" \
  http://elasticsearch:9200/_security/user/${LOGSTASH_USER} \
  -d "{\"password\":\"${LOGSTASH_PASSWORD}\",\"roles\":[\"logstash_writer\"]}" || true
